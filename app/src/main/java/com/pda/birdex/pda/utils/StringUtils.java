...
//	}
    ...
    /**
     * 判断给定字符串是否空白串。 字符串是否空白串。 空白串是指由空格、制表符、回车符、换行符组成的字符串
    ...
    /**
     * @param inputText
     * @return java正则表达式判断是否为电话号码
     */
    public static boolean isPhone(String inputText) {
        if (null == inputText || inputText.trim().length() == 0) {
            return false;
        }
        return phone.matcher(inputText).matches();
    }
    ...
    public static String millisToTimeString(long millis) {
        ...
        if (hour > 0) {
            time = (negative ? "-" : "") + ":" + df.format(min) + ":" + df.format(sec);
        } else {
    ...
    // 将 String 数组 拆分成 List  例如： ["1","2"]
    public static List<String> spiltStringArray(String arrayStr) {
        String str1 = arrayStr.substring(1, arrayStr.length() - 1); // 拆分成了 "1","2"
